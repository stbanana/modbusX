...
另外想了解modbus协议有[modbus协议介绍](https://www.yono233.cn/posts/shoot/24_7_26_modbus协议介绍(未完成))
...
我过去实现的modbus库虽然非常简陋(共约200行)，但是已经满足了上述需求，并且已经用于生产环境。但采用宏翻译表，导致ROM消耗巨大(当然会带来无与伦比的时间优化)，在工业领域尽可能全静态当然是好的设计，但我希望无论静态或动态都有比较好的支持。同时，既然决定做成开源库的形式，该有的机制慢慢都要加上去。
...
> 2. **如果映射到的内部内存变量相同，那么回调处理也应该相同**，因为库会最终将整个变量期望修改的值以void*形式传入写时回调，写时回调应当再翻译为对应的数据，详见例程 [在windows平台的从机例子](Example/win_test/RTU_Smain.c)
...
> 1. 数据模型的**寄存器地址必须递增**，库内使用二分法查找以提高查找效率
> 2. **如果映射到的内部内存变量相同，那么回调处理也应该相同**，因为库会最终将整个变量期望修改的值以void*形式传入写时回调，写时回调应当再翻译为对应的数据，详见例程 [在windows平台的主机例子](Example/win_test/RTU_Mmain.c)
