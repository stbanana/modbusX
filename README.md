...
我对协议库的理解是：只需要另外实现数据的流入和流出 port，并最多另外定时调用一个时基函数，就可以驱动起整个库(时间片轮询)。
...
> 5. 🗽**寄存器地址表与内存地址任意映射**：**本库的重要特性，解决 modbus 虚拟地址与实际变量之间的映射关系，使得开发更为简单且自由。维护更加直观方便。真正实现一个声明式的关系**。整个库的使用完全围绕这个映射表展开，未来的维护同样只需维护映射表。
...
基于以下原因，本库永不提供可编译的完整嵌入式例程。

**如果涉及到嵌入式实际工程，那就要预设一种硬件状态，有无流控？485ic 选型引起的不同切换方向时间？使用 DMA 吗？等等这些问题，应该为每个项目单独设计，没有统一解。完整嵌入式工程是定制化一次性程序。**

在嵌入式应用中遇到了本库无关的问题，例如485为什么没数据。
请添加 QQ 群 **833761938**，或者邮件咨询作者。不要提 issue。
...
首先实现通信端口的 port 函数类似如下，需要实现数据流入流出的接口。
